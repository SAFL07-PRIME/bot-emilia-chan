...
// Owner numbers
const ownerNumber = ["6283876338415"];
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("RIK~")) {
    console.log("❌ Invalid RIK session format");
    process.exit(1);
  }
  ...
  console.log("♻️ RIK session restored successfully");
}
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      try {
        ...
        sock.sendMessage(ownerNumber[0] + '6283876338415@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
